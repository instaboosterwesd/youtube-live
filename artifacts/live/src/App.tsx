...
function LivePage({workspace}:{workspace:ReturnType<typeof useWorkspace>}) {
  ...
  const start=async(c:LiveChannel)=>{if(busy.includes(c.id))return;setBusy(ids=>[...ids,c.id]);try{const category=data.groups.find(g=>g.id===c.groupId)?.name;if(!category)throw new Error("Choose a video category before starting.");const result=await startStream({streamId:c.id,ingestUrl:c.streamUrl,category});if(result.status!=="running")throw new Error(result.message);update({channels:data.channels.map(x=>x.id===c.id?{...x,status:"live",viewers:0,startedAt:now()}:x)},{message:`${c.title} is now streaming from the ${category} video`,type:"live"});}catch(error){workspace.setToast(error instanceof Error?error.message:"Could not start the real stream.");}finally{setBusy(ids=>ids.filter(id=>id!==c.id));}};
