...
import {
  Activity as ActivityIcon, Archive, ArrowRight, BarChart3, BookOpen, Check,
  ChevronDown, CircleHelp, Clipboard, Clock3, Copy, Download, Eye, EyeOff,
  FileVideo, FolderOpen, Gauge, KeyRound, LayoutDashboard, Link2, Menu, MonitorPlay,
  MoreHorizontal, Pencil, Play, Plus, Radio, RefreshCw, Search, Settings, ShieldCheck,
  Square, Trash2, Upload, Users, Video, X, Zap,
} from "lucide-react";
...
type LiveChannel = {
  ...
  thumbnailColor: string; createdAt: string;
};
type VideoItem = {
  id: string; title: string; duration: string; status: VideoStatus; groupId: string;
  sourceUrl: string; thumbnailColor: string; views: number; createdAt: string;
};
...
const seed: DataState = {
  channels: [
    { id:"ch-studio", title:"Studio dispatch · episode 18", platform:"YouTube", status:"live", groupId:"gd5", streamUrl:"rtmp://a.rtmp.youtube.com/live2", streamKey:"yt-7d91-4bb3-2e18", viewers:247, startedAt:new Date(Date.now()-38*60000).toISOString(), thumbnailColor:"#2c8b88", createdAt:now() },
    { id:"ch-cutdown", title:"The cutdown room", platform:"Twitch", status:"scheduled", groupId:"gd5", streamUrl:"rtmp://live.twitch.tv/app", streamKey:"live_1948a0be6b", viewers:0, startedAt:null, thumbnailColor:"#9a6591", createdAt:now() },
    { id:"ch-archive", title:"Members only / monthly notes", platform:"YouTube", status:"stopped", groupId:"", streamUrl:"rtmp://a.rtmp.youtube.com/live2", streamKey:"yt-archive-9328", viewers:0, startedAt:null, thumbnailColor:"#607a98", createdAt:now() },
  ],
  videos: [
    { id:"vid-1", title:"How we build a live show in one afternoon", duration:"38:42", status:"published", groupId:"gd5", sourceUrl:"", thumbnailColor:"#2c8b88", views:1842, createdAt:now() },
    { id:"vid-2", title:"Studio dispatch · episode 17", duration:"54:09", status:"published", groupId:"gd5", sourceUrl:"", thumbnailColor:"#da814b", views:921, createdAt:now() },
    { id:"vid-3", title:"Field notes: a quieter kind of production", duration:"12:28", status:"draft", groupId:"", sourceUrl:"", thumbnailColor:"#788d52", views:0, createdAt:now() },
    { id:"vid-4", title:"Five cuts that changed the edit", duration:"21:16", status:"archived", groupId:"", sourceUrl:"", thumbnailColor:"#9a6591", views:486, createdAt:now() },
  ],
  groups: [{ id:"gd5", name:"GD5", description:"The main studio dispatch series.", videoIds:["vid-1","vid-2"], createdAt:now() }],
  activities: [
    { id:"a-1", type:"live", message:"Studio dispatch · episode 18 went live on YouTube", time:"12 minutes ago" },
    { id:"a-2", type:"video", message:"How we build a live show in one afternoon was published", time:"Yesterday, 4:18 PM" },
    { id:"a-3", type:"edit", message:"The cutdown room was scheduled for Friday", time:"Yesterday, 2:06 PM" },
    { id:"a-4", type:"group", message:"2 videos added to GD5", time:"Tue, 11:42 AM" },
  ],
};

function useWorkspace() {
  const [data, setData] = useState<DataState>(() => {
    try { return JSON.parse(localStorage.getItem("signal-desk-data") || "null") || seed; } catch { return seed; }
  });
  const [user, setUser] = useState(() => localStorage.getItem("signal-desk-user") || "");
  const [toast, setToast] = useState("");
  useEffect(() => { localStorage.setItem("signal-desk-data", JSON.stringify(data)); }, [data]);
...
function ChannelModal({ channel, groups, onSave, onClose }: {channel?:LiveChannel; groups:VideoGroup[]; onSave:(c:LiveChannel)=>void; onClose:()=>void}) {
  const [form,setForm] = useState({title:channel?.title||"", platform:channel?.platform||"YouTube", status:channel?.status||"scheduled", groupId:channel?.groupId||"", streamUrl:channel?.streamUrl||"rtmp://a.rtmp.youtube.com/live2", streamKey:channel?.streamKey||"", thumbnailColor:channel?.thumbnailColor||colors[0]});
  const set=(key:string,value:string)=>setForm(f=>({...f,[key]:value}));
  const submit=(e:FormEvent)=>{e.preventDefault(); if(!form.title.trim() || !form.streamKey.trim()) return; onSave({id:channel?.id||uid("ch"), title:form.title.trim(), platform:form.platform, status:form.status as LiveStatus, groupId:form.groupId, streamUrl:form.streamUrl, streamKey:form.streamKey, viewers:channel?.viewers||0, startedAt:channel?.startedAt||null, thumbnailColor:form.thumbnailColor, createdAt:channel?.createdAt||now()});};
  return <Modal title={channel ? "Edit channel" : "Add live channel"} onClose={onClose} footer={<><button className="button ghost" onClick={onClose} data-testid="button-cancel-channel">Cancel</button><button className="button" type="submit" form="channel-form" data-testid="button-save-channel">{channel ? "Save changes" : "Add channel"} <Check size={14}/></button></>}><form id="channel-form" onSubmit={submit}><div className="form-grid"><div className="field full"><label>Channel title</label><input autoFocus required value={form.title} onChange={e=>set("title",e.target.value)} placeholder="Friday night dispatch" data-testid="input-channel-title"/></div><div className="field"><label>Platform</label><select value={form.platform} onChange={e=>set("platform",e.target.value)} data-testid="select-channel-platform"><option>YouTube</option><option>Twitch</option><option>Vimeo</option><option>Custom RTMP</option></select></div><div className="field"><label>Initial status</label><select value={form.status} onChange={e=>set("status",e.target.value)} data-testid="select-channel-status"><option value="scheduled">Scheduled</option><option value="stopped">Stopped</option></select></div><div className="field"><label>Group assignment</label><select value={form.groupId} onChange={e=>set("groupId",e.target.value)} data-testid="select-channel-group"><option value="">No group</option>{groups.map(g=><option key={g.id} value={g.id}>{g.name}</option>)}</select></div><div className="field"><label>Thumbnail color</label><select value={form.thumbnailColor} onChange={e=>set("thumbnailColor",e.target.value)} data-testid="select-channel-color">{colors.map(c=><option value={c} key={c}>{c}</option>)}</select></div><div className="field full"><label>Stream URL</label><input value={form.streamUrl} onChange={e=>set("streamUrl",e.target.value)} data-testid="input-stream-url"/></div><div className="field full"><label>Stream key</label><input required value={form.streamKey} onChange={e=>set("streamKey",e.target.value)} placeholder="Paste your key here" data-testid="input-stream-key"/></div></div><div className="form-note"><ShieldCheck size={14} style={{verticalAlign:"-3px",marginRight:6}}/>Keys are stored only in this browser. They are never sent to a server.</div></form></Modal>;
}
...
function LivePage({workspace}:{workspace:ReturnType<typeof useWorkspace>}) {
  const {data,update}=workspace; const [editing,setEditing]=useState<LiveChannel|undefined>(); const [showForm,setShowForm]=useState(false); const [deleting,setDeleting]=useState<LiveChannel|undefined>(); const [revealed,setRevealed]=useState<string[]>([]);
  ...
  const start=(c:LiveChannel)=>update({channels:data.channels.map(x=>x.id===c.id?{...x,status:"live",viewers:Math.floor(55+Math.random()*230),startedAt:now()}:x)},{message:`${c.title} is now live on ${c.platform}`,type:"live"});
  const stop=(c:LiveChannel)=>update({channels:data.channels.map(x=>x.id===c.id?{...x,status:"stopped",viewers:0}:x)},{message:`${c.title} was taken off air`,type:"edit"});
  ...
  return <AppShell title="Live channels" workspace={workspace}><div className="page"><div className="page-head"><div><p className="eyebrow">Broadcast operations</p><h1>Live channels</h1><p className="subtle">Prepare your destinations, then take the room live with confidence.</p></div><button className="button" onClick={()=>{setEditing(undefined);setShowForm(true)}} data-testid="button-add-channel"><Plus size={16}/> Add channel</button></div>
    <div className="card section-card"><div className="section-head"><div><h2 className="section-title">{data.channels.length} channel{data.channels.length===1?"":"s"}</h2><p className="subtle" style={{margin:"5px 0 0",fontSize:11}}>{data.channels.filter(c=>c.status==="live").length} currently broadcasting · {data.channels.filter(c=>c.status==="scheduled").length} scheduled</p></div><div className="status live"><span className="status-dot"/>{data.channels.filter(c=>c.status==="live").length ? "Room monitored" : "Room quiet"}</div></div>{data.channels.length===0?<EmptyState icon={<MonitorPlay size={21}/>} title="Your live room is empty" copy="Add a destination to start preparing your first broadcast." action="Add first channel" onClick={()=>setShowForm(true)}/>:<div className="table-wrap"><table className="data-table"><thead><tr><th>Channel</th><th>Platform</th><th>Status</th><th>Audience</th><th>Group</th><th>Stream key</th><th/></tr></thead><tbody>{data.channels.map(c=><tr key={c.id} data-testid={`row-channel-${c.id}`}><td><div style={{display:"flex",alignItems:"center",gap:10}}><div className="thumb" style={{background:c.thumbnailColor,width:34,height:34}}><Radio size={14}/></div><div><div className="table-title">{c.title}</div><div className="table-sub">{c.status==="live" ? `Live for ${fmtTime(c.startedAt)}` : c.platform==="Custom RTMP" ? c.streamUrl : "Ready to broadcast"}</div></div></div></td><td><span className="mono" style={{fontSize:11}}>{c.platform}</span></td><td><div className={`status ${c.status}`}><span className="status-dot"/>{c.status}</div></td><td><span className="mono">{c.status==="live"?fmtNumber(c.viewers):"—"}</span></td><td><span className="table-sub">{groupsById[c.groupId]||"Unassigned"}</span></td><td><div className="key-box" style={{margin:0,maxWidth:190}}><code>{revealed.includes(c.id)?c.streamKey:"••••••••••••"}</code><button className="icon-button" style={{width:28,height:28}} onClick={()=>setRevealed(r=>r.includes(c.id)?r.filter(x=>x!==c.id):[...r,c.id])} title={revealed.includes(c.id)?"Hide key":"Reveal key"} data-testid={`button-reveal-key-${c.id}`}>{revealed.includes(c.id)?<EyeOff size={13}/>:<Eye size={13}/>}</button><button className="icon-button" style={{width:28,height:28}} onClick={()=>{navigator.clipboard?.writeText(c.streamKey);workspace.setToast("Stream key copied")}} title="Copy stream key" data-testid={`button-copy-key-${c.id}`}><Copy size={13}/></button></div></td><td><div className="actions">{c.status==="live"?<button className="button warn small" onClick={()=>stop(c)} data-testid={`button-stop-${c.id}`}><Square size={12}/> Stop</button>:<button className="button secondary small" onClick={()=>start(c)} data-testid={`button-start-${c.id}`}><Play size={12}/> Start</button>}<button className="icon-button" style={{width:30,height:30}} onClick={()=>{setEditing(c);setShowForm(true)}} title="Edit channel" data-testid={`button-edit-channel-${c.id}`}><Pencil size={13}/></button><button className="icon-button" style={{width:30,height:30}} onClick={()=>setDeleting(c)} title="Delete channel" data-testid={`button-delete-channel-${c.id}`}><Trash2 size={13}/></button></div></td></tr>)}</tbody></table></div>}</div>
    <div className="card section-card" style={{marginTop:18}}><div className="section-head"><div><h2 className="section-title">Signal checklist</h2><p className="subtle" style={{margin:"5px 0 0",fontSize:11}}>A few calm checks before you go on air.</p></div><Clipboard size={17} color="#6c8b83"/></div><div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(190px,1fr))",gap:10}}>{["Stream keys are saved locally","At least one destination is ready","Live audience is visible above"].map((t,i)=><div key={t} style={{display:"flex",gap:9,alignItems:"center",fontSize:11,color:"#60736c",padding:11,background:"#f5f8f1",borderRadius:8}}><span style={{width:20,height:20,borderRadius:"50%",display:"grid",placeItems:"center",background:"#dcefe1",color:"#2a7a72"}}><Check size={12}/></span>{t}</div>)}</div></div>
  </div>{showForm&&<ChannelModal channel={editing} groups={data.groups} onSave={save} onClose={()=>{setShowForm(false);setEditing(undefined)}}/>}{deleting&&<ConfirmModal title="Delete this channel?" copy={`“${deleting.title}” and its stream settings will be removed from this workspace. Any live signal must be stopped first.`} onClose={()=>setDeleting(undefined)} onConfirm={()=>{update({channels:data.channels.filter(c=>c.id!==deleting.id)},{message:`${deleting.title} was deleted`,type:"edit"});setDeleting(undefined)}}/>}</AppShell>;
}

function VideoModal({video,groups,onSave,onClose}:{video?:VideoItem;groups:VideoGroup[];onSave:(v:VideoItem)=>void;onClose:()=>void}) {
  const [form,setForm]=useState({title:video?.title||"",duration:video?.duration||"",status:video?.status||"draft",groupId:video?.groupId||"",sourceUrl:video?.sourceUrl||"",thumbnailColor:video?.thumbnailColor||colors[1]});
  const set=(key:string,value:string)=>setForm(f=>({...f,[key]:value}));
  const submit=(e:FormEvent)=>{e.preventDefault();if(!form.title.trim())return;onSave({id:video?.id||uid("vid"),title:form.title.trim(),duration:form.duration||"00:00",status:form.status as VideoStatus,groupId:form.groupId,sourceUrl:form.sourceUrl,thumbnailColor:form.thumbnailColor,views:video?.views||0,createdAt:video?.createdAt||now()})};
  return <Modal title={video?"Edit video":"Add video"} onClose={onClose} footer={<><button className="button ghost" onClick={onClose} data-testid="button-cancel-video">Cancel</button><button className="button" type="submit" form="video-form" data-testid="button-save-video">{video?"Save changes":"Add video"} <Check size={14}/></button></>}><form id="video-form" onSubmit={submit}><div className="form-grid"><div className="field full"><label>Video title</label><input autoFocus required value={form.title} onChange={e=>set("title",e.target.value)} placeholder="A thoughtful title for the archive" data-testid="input-video-title"/></div><div className="field"><label>Duration</label><input value={form.duration} onChange={e=>set("duration",e.target.value)} placeholder="24:18" data-testid="input-video-duration"/></div><div className="field"><label>Library status</label><select value={form.status} onChange={e=>set("status",e.target.value)} data-testid="select-video-status"><option value="draft">Draft</option><option value="published">Published</option><option value="archived">Archived</option></select></div><div className="field"><label>Group assignment</label><select value={form.groupId} onChange={e=>set("groupId",e.target.value)} data-testid="select-video-group"><option value="">No group</option>{groups.map(g=><option value={g.id} key={g.id}>{g.name}</option>)}</select></div><div className="field"><label>Thumbnail color</label><select value={form.thumbnailColor} onChange={e=>set("thumbnailColor",e.target.value)} data-testid="select-video-color">{colors.map(c=><option value={c} key={c}>{c}</option>)}</select></div><div className="field full"><label>Local video URL <span style={{textTransform:"none",letterSpacing:0,color:"#99a39e"}}>(optional)</span></label><input value={form.sourceUrl} onChange={e=>set("sourceUrl",e.target.value)} placeholder="file:///… or a local preview URL" data-testid="input-video-source"/></div></div><div className="form-note"><Upload size={14} style={{verticalAlign:"-3px",marginRight:6}}/>For a local preview, paste a browser-readable URL. Upload support can be added later without changing your library metadata.</div></form></Modal>;
}
...
function VideosPage({workspace}:{workspace:ReturnType<typeof useWorkspace>}) {
  ...
  const [tab,setTab]=useState<"library"|"groups">("library"); const [videoModal,setVideoModal]=useState(false);
  const [editing,setEditing]=useState<VideoItem|undefined>(); const [groupModal,setGroupModal]=useState(false);
  ...
  const saveVideo=(v:VideoItem)=>{const exists=data.videos.some(x=>x.id===v.id);let groups=data.groups.map(g=>({...g,videoIds:g.videoIds.filter(id=>id!==v.id)}));if(v.groupId)groups=groups.map(g=>g.id===v.groupId?{...g,videoIds:[...g.videoIds,v.id]}:g);update({videos:exists?data.videos.map(x=>x.id===v.id?v:x):[v,...data.videos],groups},{message:exists?`${v.title} was updated`:`${v.title} was added to the library`,type:"video"});setVideoModal(false);setEditing(undefined);};
  ...
  const library=<div className="card section-card"><div className="section-head"><div><h2 className="section-title">{filtered.length} video{filtered.length===1?"":"s"}</h2><p className="subtle" style={{margin:"5px 0 0",fontSize:11}}>{search||status!=="all"||group!=="all"?"Filtered library":"Your local media index"}</p></div><div className="table-sub">{fmtNumber(data.videos.reduce((a,v)=>a+v.views,0))} total views</div></div>{filtered.length===0?<EmptyState icon={<Search size={21}/>} title="No videos found" copy="Try a different search, or add a new piece to your library." action="Add video" onClick={()=>setVideoModal(true)}/>:<div className="table-wrap"><table className="data-table"><thead><tr><th>Video</th><th>Status</th><th>Group</th><th>Views</th><th>Source</th><th/></tr></thead><tbody>{filtered.map(v=><tr key={v.id} data-testid={`row-video-${v.id}`}><td><div style={{display:"flex",alignItems:"center",gap:10}}><div className="thumb" style={{background:v.thumbnailColor,width:52,height:34}}><Video size={14}/><span style={{fontSize:9,marginLeft:-3}}>{v.duration}</span></div><div><div className="table-title">{v.title}</div><div className="table-sub">Added {new Date(v.createdAt).toLocaleDateString()}</div></div></div></td><td><span className={`status ${v.status==="published"?"live":v.status==="draft"?"scheduled":"stopped"}`}><span className="status-dot"/>{v.status}</span></td><td><span className="table-sub">{data.groups.find(g=>g.id===v.groupId)?.name||"Unassigned"}</span></td><td><span className="mono">{fmtNumber(v.views)}</span></td><td>{v.sourceUrl?<a href={v.sourceUrl} target="_blank" rel="noreferrer" className="section-link" data-testid={`link-source-${v.id}`}><Link2 size={12} style={{verticalAlign:"-2px"}}/> Play</a>:<span className="table-sub">Not attached</span>}</td><td><div className="actions"><button className="icon-button" style={{width:30,height:30}} onClick={()=>{setEditing(v);setVideoModal(true)}} data-testid={`button-edit-video-${v.id}`}><Pencil size={13}/></button><button className="icon-button" style={{width:30,height:30}} onClick={()=>setDeleting({kind:"video",id:v.id,name:v.title})} data-testid={`button-delete-video-${v.id}`}><Trash2 size={13}/></button></div></td></tr>)}</tbody></table></div>}</div>;
  const groups=<div>{data.groups.length===0?<div className="card"><EmptyState icon={<FolderOpen size={21}/>} title="No groups yet" copy="Create a group to organize videos into a series or collection." action="Create group" onClick={()=>setGroupModal(true)}/></div>:<div className="group-grid">{data.groups.map(g=><div className="card group-card" key={g.id} data-testid={`card-group-${g.id}`}><h3>{g.name}</h3><p>{g.description||"No description yet."}</p><div className="group-foot"><span>{g.videoIds.length} video{g.videoIds.length===1?"":"s"}</span><div style={{display:"flex",gap:7}}><button onClick={()=>{setEditingGroup(g);setGroupModal(true)}} className="section-link" data-testid={`button-edit-group-${g.id}`}>Edit</button><button onClick={()=>setDeleting({kind:"group",id:g.id,name:g.name})} className="section-link" style={{color:"#a05b45"}} data-testid={`button-delete-group-${g.id}`}>Delete</button></div></div></div>)}</div>}</div>;
  return <AppShell title="Video library" workspace={workspace}><div className="page"><div className="page-head"><div><p className="eyebrow">Archive & distribution</p><h1>Video library</h1><p className="subtle">Give every cut a home, a status, and a way back into the room.</p></div><div style={{display:"flex",gap:8}}>{tab==="groups"&&<button className="button secondary" onClick={()=>setGroupModal(true)} data-testid="button-add-group"><Plus size={15}/> New group</button>}<button className="button" onClick={()=>{setEditing(undefined);setVideoModal(true)}} data-testid="button-add-video"><Plus size={15}/> Add video</button></div></div><div className="toolbar"><div className="filter-row"><button className={`button small ${tab==="library"?"":"ghost"}`} onClick={()=>setTab("library")} data-testid="button-tab-library"><FileVideo size={13}/> Library</button><button className={`button small ${tab==="groups"?"":"ghost"}`} onClick={()=>setTab("groups")} data-testid="button-tab-groups"><FolderOpen size={13}/> Groups</button></div>{tab==="library"&&<div className="filter-row"><div className="input-wrap"><Search size={14} color="#899791"/><input type="search" value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search videos…" data-testid="input-search-videos"/></div><select value={status} onChange={e=>setStatus(e.target.value)} data-testid="select-filter-status"><option value="all">All statuses</option><option value="published">Published</option><option value="draft">Draft</option><option value="archived">Archived</option></select><select value={group} onChange={e=>setGroup(e.target.value)} data-testid="select-filter-group"><option value="all">All groups</option>{data.groups.map(g=><option value={g.id} key={g.id}>{g.name}</option>)}</select></div>}</div>{tab==="library"?library:groups}</div>{videoModal&&<VideoModal video={editing} groups={data.groups} onSave={saveVideo} onClose={()=>{setVideoModal(false);setEditing(undefined)}}/>}{groupModal&&<GroupModal group={editingGroup} onSave={saveGroup} onClose={()=>{setGroupModal(false);setEditingGroup(undefined)}}/>}{deleting&&<ConfirmModal title={`Delete this ${deleting.kind}?`} copy={`“${deleting.name}” will be removed from the ${deleting.kind==="video"?"library":"workspace"}. ${deleting.kind==="group"?"Videos inside it will remain in your library.":""}`} onClose={()=>setDeleting(undefined)} onConfirm={remove}/>}</AppShell>;
}
