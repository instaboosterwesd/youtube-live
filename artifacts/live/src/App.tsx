...
import {
  ...
  Copy, Download, Eye, EyeOff, FileVideo, FolderOpen, Gauge, LayoutDashboard,
...
function ChannelModal({ channel, groups, onSave, onClose }: {channel?:LiveChannel; groups:VideoGroup[]; onSave:(c:LiveChannel)=>void; onClose:()=>void}) {
  const [form,setForm] = useState({groupId:channel?.groupId||"", streamUrl:channel?.streamUrl||"rtmp://a.rtmp.youtube.com/live2", streamKey:channel?.streamKey||""});
  const set=(key:string,value:string)=>setForm(f=>({...f,[key]:value}));
  const submit=(e:FormEvent)=>{e.preventDefault(); if(!form.streamUrl.trim() || !form.streamKey.trim() || !form.groupId) return; const platform=platformFromUrl(form.streamUrl); onSave({id:channel?.id||uid("ch"), title:channel?.title||`${platform} channel`, platform, status:channel?.status||"stopped", groupId:form.groupId, streamUrl:form.streamUrl.trim(), streamKey:form.streamKey.trim(), viewers:channel?.viewers||0, startedAt:channel?.startedAt||null, thumbnailColor:channel?.thumbnailColor||colors[0], createdAt:channel?.createdAt||now()});};
  return <Modal title={channel ? "Update channel" : "Add live channel"} onClose={onClose} footer={<><button className="button ghost" onClick={onClose} data-testid="button-cancel-channel">Cancel</button><button className="button" type="submit" form="channel-form" data-testid="button-save-channel">{channel ? "Save changes" : "Add channel"} <Check size={14}/></button></>}><form id="channel-form" onSubmit={submit}><div className="form-grid"><div className="field full"><label>Stream URL</label><input autoFocus required value={form.streamUrl} onChange={e=>set("streamUrl",e.target.value)} placeholder="rtmp://a.rtmp.youtube.com/live2" data-testid="input-stream-url"/></div><div className="field full"><label>Stream key</label><input required value={form.streamKey} onChange={e=>set("streamKey",e.target.value)} placeholder="Paste your key here" data-testid="input-stream-key"/></div><div className="field full"><label>Video category</label><select required value={form.groupId} onChange={e=>set("groupId",e.target.value)} data-testid="select-channel-group"><option value="">Select a category</option>{groups.map(g=><option key={g.id} value={g.id}>{g.name}</option>)}</select></div></div><div className="form-note"><ShieldCheck size={14} style={{verticalAlign:"-3px",marginRight:6}}/>The platform is detected from the URL. Your key is stored only in this browser and never sent to a server.</div></form></Modal>;
}
...
function VideoModal({video,groups,defaultGroupId="",onSave,onClose}:{video?:VideoItem;groups:VideoGroup[];defaultGroupId?:string;onSave:(v:VideoItem)=>void;onClose:()=>void}) {
  ...
  return <Modal title="Add video" onClose={onClose} footer={<><button className="button ghost" onClick={onClose} data-testid="button-cancel-video">Cancel</button><button className="button" type="submit" form="video-form" data-testid="button-save-video">Add video <Check size={14}/></button></>}><form id="video-form" onSubmit={submit}><div className="form-grid"><div className="field full"><label>Choose video file</label><input autoFocus required={!form.sourceUrl} type="file" accept="video/*" onChange={e=>{const file=e.target.files?.[0];if(!file)return;setFileName(file.name);setForm(f=>({...f,title:f.title||file.name.replace(/\.[^.]+$/,""),sourceUrl:URL.createObjectURL(file)}))}} data-testid="input-video-file"/>{fileName&&<span className="file-picked"><Check size={13}/> {fileName}</span>}</div><div className="field full"><label>Video title</label><input required value={form.title} onChange={e=>set("title",e.target.value)} placeholder="Title for this video" data-testid="input-video-title"/></div><div className="field"><label>Video category</label><select required value={form.groupId} onChange={e=>set("groupId",e.target.value)} data-testid="select-video-group"><option value="">Select a category</option>{groups.map(g=><option value={g.id} key={g.id}>{g.name}</option>)}</select></div><div className="field"><label>Duration</label><input value={form.duration} onChange={e=>set("duration",e.target.value)} placeholder="24:18" data-testid="input-video-duration"/></div></div><div className="form-note"><Upload size={14} style={{verticalAlign:"-3px",marginRight:6}}/>Choose a video file, give it a category, and add it to the library. The file stays in this browser session.</div></form></Modal>;
}
...
function VideosPage({workspace}:{workspace:ReturnType<typeof useWorkspace>}) {
  ...
  const [tab,setTab]=useState<"library"|"groups">("groups"); const [videoModal,setVideoModal]=useState(false);
